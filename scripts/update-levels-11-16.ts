...
function cleanMarkdownContent(content: string, dayNumber: number): string {
  ...
  const dayNames: { [key: number]: string } = {
    ...
    14: '高阶函数 (Higher Order Functions)',
    15: '类型错误 (Python Type Errors)',
    16: '日期时间 (Python DateTime)'
  };
...
async function updateLevels11to16() {
  console.log('🔄 开始清理并更新 Day 11-16 的内容...\n');
...
updateLevels11to16().catch(console.error);
